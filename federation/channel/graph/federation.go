...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Channel":
		resolverName, err := entityResolverNameForChannel(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Channel": %w`, err)
		}
		switch resolverName {

		case "findChannelByContentID":
			id0, err := ec.unmarshalNString2string(ctx, rep["content_id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findChannelByContentID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindChannelByContentID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Channel": %w`, err)
			}
...
func (ec *executionContext) resolveManyEntities(
	...
	switch typeName {
	case "Channel":
		// Extract all content_ids from the representations
		contentIDs := make([]string, len(reps))
		for i, rep := range reps {
			id, err := ec.unmarshalNString2string(ctx, rep.entity["content_id"])
			if err != nil {
				return fmt.Errorf(`unmarshalling content_id for Channel: %w`, err)
			}
			contentIDs[i] = id
		}

		// Batch resolve all channels at once using our helper function
		entities, err := BatchFindChannelsByContentIDs(contentIDs)
		if err != nil {
			return fmt.Errorf(`resolving Entities "Channel": %w`, err)
		}
...
func entityResolverNameForChannel(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"content_id\" for Channel", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Channel", ErrTypeNotFound))
			break
		}
		return "findChannelByContentID", nil
	}
	return "", fmt.Errorf("%w for Channel due to %v", ErrTypeNotFound,
